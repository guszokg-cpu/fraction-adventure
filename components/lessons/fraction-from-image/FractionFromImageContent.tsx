...
export function FractionFromImageContent() {
  return (
    <div className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {sameFractionExamples.map((example) => (
              <div key={example.id} className="rounded-xl border border-brand-100 bg-white p-4 text-center">
                ...
                <div className="text-3xl font-extrabold text-brand-900">3/4</div>
              </div>
            ))}
          </div>
          <div className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-center text-sm font-bold text-amber-700">
            แม้ภาพต่างกัน แต่ถ้าระบาย 3 จาก 4 ส่วนเท่ากัน ก็เขียนเป็น 3/4 เหมือนกัน
          </div>
        </div>
      </Card>
      ...
      <LessonActionBar meta={fractionFromImageMeta} practiceCount={10} />
    </div>
  );
}
