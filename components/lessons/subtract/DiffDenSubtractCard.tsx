...
function makeProblem(): Problem {
  for (let guard = 0; guard < 60; guard++) {
    ...
    // ตัวตั้งต้องมากกว่าตัวลบ (ผลลัพธ์เป็นบวก ไม่เป็นศูนย์)
    if (na * (l / da) > nb * (l / db)) return { na, da, nb, db };
  }
  return { na: 3, da: 4, nb: 1, db: 2 };
}
...
export function DiffDenSubtractCard() {
  const [problem, setProblem] = useState<Problem>({ na: 3, da: 4, nb: 1, db: 2 });
  ...
  // ตัวส่วนต้องมากกว่า 1 และผลลัพธ์ต้องเป็นบวก — นอกนั้นพิมพ์ได้ทุกค่า
  let warning: string | null = null;
  if (da < 2 || db < 2) warning = "ตัวส่วนต้องมากกว่า 1";
  else if (diff <= 0) warning = "ตัวตั้งต้องมากกว่าตัวลบ (ผลลัพธ์ต้องมากกว่า 0)";
  const valid = warning === null;

  // ถ้าพหุคูณสั้นพอ แสดงเป็นแถวพหุคูณ; ถ้ายาวเกินไปสลับไปวิธีหารสั้น
  const showMultiples = l / da <= 6 && l / db <= 6;
  // ถ้าตัวส่วนเล็กพอ ใช้แท่งแบ่งช่อง; ไม่งั้นใช้แท่งสัดส่วนต่อเนื่อง
  const smallBars = l <= 20 && ca <= l;
    ...
    <Card className="overflow-hidden p-0">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        <div className="flex items-center justify-center gap-4 rounded-2xl bg-emerald-50 px-4 py-4">
          <FractionStack top={na} bottom={da} className="text-3xl text-emerald-600 sm:text-4xl" />
          <span className="text-3xl font-extrabold text-slate-400 sm:text-4xl">−</span>
          <FractionStack top={nb} bottom={db} className="text-3xl text-rose-500 sm:text-4xl" />
          <span className="text-3xl font-extrabold text-slate-400 sm:text-4xl">=</span>
          {valid && stage >= 2 ? (
            <FractionStack top={diff} bottom={l} className="text-3xl text-emerald-700 sm:text-4xl" />
          ) : (
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-200 p-3 sm:p-4">
          ...
          <FractionInput
            ...
            maxNumerator={99}
            maxDenominator={99}
            colorClass="border-emerald-300 focus:border-emerald-500 focus:ring-emerald-100 bg-emerald-500"
          />
          ...
          <FractionInput
            ...
            maxNumerator={99}
            maxDenominator={99}
            colorClass="border-rose-300 focus:border-rose-500 focus:ring-rose-100 bg-rose-500"
          />
          <button
            onClick={randomize}
        ...
        {valid && (
          <>
              ...
              {showMultiples ? (
                <>
                  <div className="mt-3 space-y-2">
                    <MultiplesRow den={da} l={l} color="bg-emerald-500" />
                    <MultiplesRow den={db} l={l} color="bg-rose-400" />
                  </div>
                  <p className="mt-2 text-center text-sm font-bold text-slate-600">
                    เจอกันครั้งแรกที่ <span className="text-lg font-extrabold text-emerald-700">{l}</span> → ใช้ {l} เป็นตัวส่วนร่วม
                  </p>
                </>
              ) : (
                <div className="mt-3">
                  <LcmLadder a={da} b={db} badgeClass="bg-emerald-600" resultClass="text-emerald-700" />
                </div>
              )}
            </div>
            ...
            {stage >= 1 && (
              <div className="rounded-2xl border-2 border-teal-200 bg-white p-4 sm:p-5">
                <p className="flex items-center gap-2 text-base font-extrabold text-teal-700 sm:text-lg">
                  <span className="grid h-7 w-7 place-items-center rounded-full bg-teal-500 text-sm text-white">2</span>
                ...
                <div className="mt-3 grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2 text-center">
                    <div className="flex items-center justify-center gap-2 text-xl font-extrabold sm:text-2xl">
                      <FractionStack top={na} bottom={da} className="text-emerald-600" />
                      <span className="flex flex-col items-center gap-1.5 text-base text-emerald-500">
                        <span>×{ka}</span>
                        <span>×{ka}</span>
                      </span>
                      <span className="text-slate-400">=</span>
                      <FractionStack top={ca} bottom={l} className="text-emerald-600" />
                    </div>
                    {smallBars ? (
                      <FractionShape numerator={ca} denominator={l} shape="bar" tone="emerald" className="h-9 w-full" />
                    ) : (
                      <ProportionBar num={ca} den={l} barClass="bg-emerald-400" className="h-9 w-full" />
                    )}
                  </div>
                  <div className="space-y-2 text-center">
                    <div className="flex items-center justify-center gap-2 text-xl font-extrabold sm:text-2xl">
                      <FractionStack top={nb} bottom={db} className="text-rose-500" />
                      <span className="flex flex-col items-center gap-1.5 text-base text-rose-400">
                        <span>×{kb}</span>
                        <span>×{kb}</span>
                      </span>
                      <span className="text-slate-400">=</span>
                      <FractionStack top={cb} bottom={l} className="text-rose-500" />
                    </div>
                    {smallBars ? (
                      <FractionShape numerator={cb} denominator={l} shape="bar" tone="pink" className="h-9 w-full" />
                    ) : (
                      <ProportionBar num={cb} den={l} barClass="bg-rose-400" className="h-9 w-full" />
                    )}
                  </div>
                </div>
              </div>
            ...
            {/* ขั้นที่ 3: ลบตัวเศษ */}
            {stage >= 2 && (
              <div className="rounded-2xl border-2 border-emerald-300 bg-white p-4 sm:p-5">
                <p className="flex items-center gap-2 text-base font-extrabold text-emerald-700 sm:text-lg">
                  <span className="grid h-7 w-7 place-items-center rounded-full bg-emerald-600 text-sm text-white">3</span>
                  ส่วนเท่ากันแล้ว ลบตัวเศษได้เลย
                </p>
                <div className="mt-3 flex items-center justify-center gap-3 text-2xl font-extrabold sm:text-3xl">
                  ...
                  <FractionStack top={cb} bottom={l} className="text-rose-500" />
                  <span className="text-slate-400">=</span>
                  <FractionStack top={diff} bottom={l} className="text-emerald-700" />
                </div>
                <div className="mx-auto mt-3 max-w-xl">
                  {smallBars ? (
                    <CrossOutBar filled={ca} removed={cb} denominator={l} />
                  ) : (
                    <TwoSegmentBar first={diff} second={cb} den={l} firstClass="bg-emerald-400" secondClass="bg-rose-300" className="h-11 w-full sm:h-12" />
                  )}
                  <p className="mt-2 text-center text-sm font-bold text-slate-500">
                    {smallBars ? "ขีดฆ่าชิ้นที่เอาออก" : "ส่วนสีชมพูคือส่วนที่เอาออก"} {cb} ชิ้น เหลือ {diff} ชิ้น
                  </p>
                </div>
              </div>
            )}
            ...
            {stage >= 3 && hasReduce && (
              <div className="rounded-2xl border-2 border-lime-300 bg-white p-4 sm:p-5">
                <p className="flex items-center gap-2 text-base font-extrabold text-lime-700 sm:text-lg">
                  <span className="grid h-7 w-7 place-items-center rounded-full bg-lime-600 text-sm text-white">4</span>
                  อย่าลืมย่อให้เป็นอย่างต่ำ
                </p>
                <div className="mt-3 flex items-center justify-center gap-3 text-2xl font-extrabold sm:text-3xl">
                  <FractionStack top={diff} bottom={l} className="text-emerald-700" />
                  <span className="flex flex-col items-center gap-1.5 text-base text-lime-600">
                    <span>÷{g}</span>
                    <span>÷{g}</span>
                  </span>
                  <span className="text-slate-400">=</span>
                  <FractionStack top={diff / g} bottom={l / g} className="text-lime-700" />
            ...
            {stage >= maxStage && !hasReduce && (
              <div className="flex items-center justify-center gap-2 rounded-xl bg-lime-50 px-4 py-3 text-base font-extrabold text-lime-700 sm:text-lg">
