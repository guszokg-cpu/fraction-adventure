...
function makeProblem(): Problem {
  ...
  const reduced = simplifyFraction(a * b, p * q);
  const correct: Fraction = { num: reduced.numerator, den: reduced.denominator };

  const seenKeys = new Set([`${correct.num}/${correct.den}`]);
  const wrongsRaw = [
    { num: a + b, den: p + q }, // กับดัก: บวกแทนคูณ
    { num: a * b + 1, den: p * q },
    { num: a * b, den: p * q + 1 },
    { num: a * q, den: p * b },
  ];
...
export function MultiplyTimerGameCard() {
  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className="flex items-center justify-between bg-gradient-to-r from-orange-600 to-amber-400 px-4 py-2.5 text-white">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        {finished && (
          <div className="space-y-3">
            <div className="rounded-xl bg-emerald-100 px-4 py-4 text-center">
              <p className="text-lg font-extrabold text-emerald-700 sm:text-xl">หมดเวลา! คูณถูก {score} ข้อ</p>
            </div>
            <div className="flex justify-center">
              <button
                onClick={start}
        ...
        {running && (
          <div
            className={cn(
              ...
              flash === "correct" ? "border-emerald-300 bg-emerald-50" : flash === "wrong" ? "border-rose-300 bg-rose-50" : "border-orange-100 bg-white"
            )}
          >
            <div className="flex items-center justify-center gap-3 text-3xl font-extrabold sm:text-4xl">
              <FractionStack top={problem.a} bottom={problem.p} className="text-sky-600" />
              <span className="text-slate-400">×</span>
              <FractionStack top={problem.b} bottom={problem.q} className="text-amber-600" />
              <span className="text-slate-400">= ?</span>
            </div>
        ...
        <p className="text-center text-sm font-bold text-slate-500 sm:text-base">
          คูณเศษกับเศษ ส่วนกับส่วน — ระวังตัวลวงที่เผลอบวกแทนคูณ!
        </p>
      </div>
    </Card>
  );
